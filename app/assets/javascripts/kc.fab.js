    ...
    $.kc.fab = function(el, links, options){
        ...
        base.init = function(){
            ...
            if (base.links.length > 0){
                var main_btn = base.links[0];
                color_style = (main_btn.color)? "color:"+main_btn.color+";" : "";
                bg_color_style = (main_btn.bgcolor)? "background-color:"+main_btn.bgcolor+";" : "";
                var id = (main_btn.id)? "id=\"" + main_btn.id + "\"" : "";

                var main_btn_dom = "<button " + id + " data-link-href='"+((main_btn.url)?main_btn.url:"")+"' data-link-target='"+((main_btn.target)?main_btn.target:"")+"'' class='kc_fab_main_btn' style='"+bg_color_style+"'><span style='"+color_style+"'>"+main_btn.icon+"</span></button>";
                

                var sub_fab_btns_dom = "";
                base.links.shift();
                /* Loop through the remaining links array */
                for (var i = 0; i < base.links.length; i++) {
                    var color_style = (base.links[i].color)? "color:"+base.links[i].color+";" : "";
                    var bg_color_style = (base.links[i].bgcolor)? "background-color:"+base.links[i].bgcolor+";" : "";
                    var id = (base.links[i].id)? "id=\"" + base.links[i].id + "\"" : "";
                    sub_fab_btns_dom += "<div><button " + id + " data-link-href='"+(base.links[i].url?base.links[i].url:"")+"' data-link-target='"+((base.links[i].target)?base.links[i].target:"")+"' class='sub_fab_btn' style='"+bg_color_style+"'><span style='"+color_style+"'>"+base.links[i].icon+"</span></button></div>";
